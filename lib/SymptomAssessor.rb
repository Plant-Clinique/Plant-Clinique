...
class SymptomAssessor
  ...
  def get_diagnosis(plant_trefle_id, symptom_id, cause_id, user_plant_estimate)
    ...
    plant_data = JSON.parse(Net::HTTP.get(URI.parse("https://trefle.io/api/v1/plants/#{plant_trefle_id}?token=6j4O7U0FSKM_Te6mN3aFN7TORBk0RYtU_wk5sJgkjbw")))['data']['main_species']
    api_ideal_for_plant_type = CHECK_TARGET[field](plant_data)
    if check_type:
      if CHECK_TYPE[check_type](api_ideal_for_plant_type, user_plant_estimate)
        return diagnosis
      end
    end
  end
